...
export const DisplayTranscript = ({
  ...
  const copyContent = showSegments
    ? segments
        .map(
          (s) =>
            `[${formatTime(s.from_ms)} → ${formatTime(s.to_ms)}] ${s.text}`,
        )
        .join("\n")
    : (text ?? "");
  ...
  return (
    <div
      ref={containerRef}
      className="bg-bg border border-line rounded-lg min-h-64 max-h-80 lg:min-h-96 lg:max-h-150 overflow-y-auto"
    >
        ...
        {progress && (
          <div className="flex flex-col gap-1 mt-2">
            <div className="flex justify-between text-xs text-muted">
              ...
              {progress.count != null && (
                <span className="shrink-0 ml-2">{progress.count}%</span>
              )}
            </div>
            <div
              role="progressbar"
            ...
            >
              <div
                ...
                style={{
                  width: `${progress.count != null ? progress.count : 100}%`,
                }}
              />
            </div>
          </div>
      ...
      {showSegments ? (
        ...
        text && (
          <p className="text-base leading-relaxed whitespace-pre-wrap p-4">
            {text}
          </p>
        )
      )}
